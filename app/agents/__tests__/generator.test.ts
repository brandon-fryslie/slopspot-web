...
// a face (migration 0049) sits for a self-portrait through authorSlop, which reads the
...
describe('authoring config view tolerates a cross-role persona (roll-call-f7n)', () => {
  it('parses a faced critic config — medium present, voter thresholds tolerated', async () => {
    ...
    // The critics 0049 gave a face (the Gremlin refused, so it carries no medium).
    const faced = voters.find((v) => typeof v.config.medium === 'string')
    expect(faced, 'a seeded voter should carry a portrait medium after 0049').toBeDefined()
    if (!faced) return
